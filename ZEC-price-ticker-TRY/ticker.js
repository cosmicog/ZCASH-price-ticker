...
function checkTicker() 
{
	...
	ticker.onreadystatechange = function() 
	{
		if (ticker.readyState == 4 && ticker.status == 200) 
		{
			...
			lines.push("ZEC "+FixIfNotNull(coin_usd*usd_try,4)+" ₺ \n");
			lines.push("ZEC 1h Change "+Number(coin_percent1h*usd_try).toLocaleString(window.navigator.language,options)+"\n");
			lines.push("ZEC 7d Change "+Number(coin_percent7d*usd_try).toLocaleString(window.navigator.language,options)+"\n");
			lines.push("ZEC 24h Change "+FixIfNotNull(coin_percent24h*usd_try,2)+"%\n");
			lines.push("ZEC 24h Volume "+Number(coin_volume*usd_try).toLocaleString(window.navigator.language,options)+"\n");
			lines.push("ZEC Marketcap "+Number(coin_marketcap*usd_try).toLocaleString(window.navigator.language,options)+"\n");
			lines.push("\n");
			lines.push(Date());
			var title_lines="";
