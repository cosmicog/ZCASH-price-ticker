...
function checkTicker() {
  var ticker = new XMLHttpRequest();
	...
	if (ticker.readyState == 4 && ticker.status == 200) {
		...
		chrome.browserAction.setBadgeText({text: parseFloat(coin_usd).toFixed(1)});
		var lines=["ETH price ticker (price taken from 'www.coinmarketcap.com')\n"];
    var options = { style: 'currency', currency: 'USD'};
		lines.push("\n");
		lines.push("ETH "+FixIfNotNull(coin_btc,8)+"BTC\n");
		lines.push("ETH "+FixIfNotNull(coin_usd,4)+"$\n");
		lines.push("ETH 1h Change "+Number(coin_percent1h).toLocaleString(window.navigator.language,options)+"\n");
		lines.push("ETH 7d Change "+Number(coin_percent7d).toLocaleString(window.navigator.language,options)+"\n");
		lines.push("ETH 24h Change "+FixIfNotNull(coin_percent24h,2)+"%\n");
		lines.push("ETH 24h Volume "+Number(coin_volume).toLocaleString(window.navigator.language,options)+"\n");
		lines.push("ETH Marketcap "+Number(coin_marketcap).toLocaleString(window.navigator.language,options)+"\n");
		lines.push("\n");
		lines.push(Date());
		var title_lines="";
